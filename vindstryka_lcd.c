...
static const icon_map_t _icon_map[] = {
    ...
    [VINDSTRYKA_LCD_ICON_TOP_LEFT_UNDERLINE] = {.nibble_index = 4,
                                                .bit_mask = 0x08},
    [VINDSTRYKA_LCD_ICON_TOP_UNDERLINE] = {.nibble_index = 25,
                                           .bit_mask = 0x08},
    [VINDSTRYKA_LCD_ICON_PM25] = {.nibble_index = 25, .bit_mask = 0x04},
    [VINDSTRYKA_LCD_ICON_TL_RED] = {.nibble_index = 24, .bit_mask = 0x01},
    [VINDSTRYKA_LCD_ICON_TL_RED_YELLOW] = {.nibble_index = 24,
                                           .bit_mask = 0x02},
    [VINDSTRYKA_LCD_ICON_TL_YELLOW] = {.nibble_index = 24, .bit_mask = 0x04},
    [VINDSTRYKA_LCD_ICON_TL_YELLOW_GREEN] = {.nibble_index = 24,
                                             .bit_mask = 0x08},
    [VINDSTRYKA_LCD_ICON_TL_GREEN] = {.nibble_index = 15, .bit_mask = 0x8},
    [VINDSTRYKA_LCD_ICON_BRIGHTNESS] = {.nibble_index = 8, .bit_mask = 0x01},
    [VINDSTRYKA_LCD_ICON_BRIGHTNESS_LEVEL_1] = {.nibble_index = 8,
                                                .bit_mask = 0x02},
    [VINDSTRYKA_LCD_ICON_BRIGHTNESS_LEVEL_2] = {.nibble_index = 8,
                                                .bit_mask = 0x04},
    [VINDSTRYKA_LCD_ICON_BRIGHTNESS_LEVEL_3] = {.nibble_index = 8,
                                                .bit_mask = 0x08},
    ...
    [VINDSTRYKA_LCD_ICON_FARENHEIT_ARROW] = {.nibble_index = 15,
                                             .bit_mask = 0x04},
    [VINDSTRYKA_LCD_ICON_CELSIUS] = {.nibble_index = 15, .bit_mask = 0x01},
    [VINDSTRYKA_LCD_ICON_CELSIUS_ARROW] = {.nibble_index = 15,
                                           .bit_mask = 0x02},
...
void VINDSTRYKA_LCD_SetDigit(uint8_t digit, uint8_t value) {
  ...
  // Segment data for the digit
  uint8_t upper_segments = (_lcd_digit_map[value] >> 8) & 0x07;
  uint8_t lower_segments = _lcd_digit_map[value] & 0x0F;

  // First nibble. Preserve bit 3, which may contain an icon
  _lcd_buffer[first_nibble] =
      (_lcd_buffer[first_nibble] & 0x08) | upper_segments;
  // Second nibble. No icons in this nibble, can overwrite directly
  _lcd_buffer[first_nibble + 1] = lower_segments;
}

void VINDSTRYKA_LCD_SetIcon(enum VINSTRYKA_LCD_ICON icon, uint8_t state) {
  if (state) {
    _lcd_buffer[_icon_map[icon].nibble_index] |=
        _icon_map[icon].bit_mask; // Set the bit
  } else {
    _lcd_buffer[_icon_map[icon].nibble_index] &=
        ~_icon_map[icon].bit_mask; // Clear the bit
  }
}